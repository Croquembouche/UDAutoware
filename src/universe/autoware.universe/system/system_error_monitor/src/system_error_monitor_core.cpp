    ...
    rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true))
{
  ...
  get_parameter_or<double>("data_ready_timeout", params_.data_ready_timeout, 1000.0);
  ...
  timer_ = rclcpp::create_timer(this, get_clock(), period_ns, std::bind(&AutowareErrorMonitor::onTimer, this));
}
...
{
  auto isTimeout = [this](const rclcpp::Time &last_stamp_, const double threshold) {
    if (last_stamp_.get_clock_type() != this->now().get_clock_type()) {
      // RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 5000, "clock type is different...");
      // RCLCPP_ERROR_STREAM( get_logger(), "clock type mismatch: " << last_stamp_.nanoseconds() << " vs " << this->now().nanoseconds());
      return false;
    }
    const auto time_diff = this->now() - last_stamp_;
