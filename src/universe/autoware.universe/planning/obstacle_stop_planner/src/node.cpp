  ...
  const VehicleInfo & vehicle_info, const StopParam & stop_param)
{
  ...
  // RCLCPP_ERROR_STREAM(get_logger(), "trying to find the error: input: " << input_points_ptr->header.frame_id << "trajector" << trajectory_header.frame_id);
  try {
    transform_stamped = tf_buffer_.lookupTransform(
      trajectory_header.frame_id, input_points_ptr->header.frame_id, input_points_ptr->header.stamp, rclcpp::Duration::from_seconds(30.0));
  } catch (tf2::TransformException & ex) {
    RCLCPP_ERROR_STREAM(get_logger(), "trying to find the error: input: " << ex.what());
    RCLCPP_ERROR_STREAM(
      get_logger(), "Failed to look up transform from, is this error here? " << trajectory_header.frame_id << " to "
