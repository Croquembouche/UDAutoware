  ...
  const geometry_msgs::msg::TwistWithCovarianceStamped::ConstSharedPtr vehicle_twist_ptr)
{
  ...
  if (!imu_arrived_) {
    std::cout << "IMU msg is not subscribed" << std::endl;
  ...
  //   std::cout << "imu timed out in vehicle twist callback" << std::endl;
  //   const std::string error_msg = fmt::format(
...
void GyroOdometer::callbackImu(const sensor_msgs::msg::Imu::ConstSharedPtr imu_msg_ptr)
{
  ...
  if (!vehicle_twist_arrived_) {
    std::cout << "Twist msg is not subscribed" << std::endl;
  ...
  // if (imu_dt > message_timeout_sec_) {
  //   const double current = std::abs(this->now().seconds);
  //   std::cout << "now:" << current << std::endl;
  //   std::cout << "imu timed out:" << imu_dt << std::endl;
  //   std::cout << "msg timeout sec:" << message_timeout_sec_ << std::endl;
  //   const std::string error_msg = fmt::format(
  ...
  if (!tf_imu2base_ptr) {
    std::cout << "Please publish TF for gyro." << std::endl;
    RCLCPP_ERROR(
      this->get_logger(), "Please publish TF for gyro %s to %s", output_frame_.c_str(),
  ...
  const geometry_msgs::msg::TwistWithCovarianceStamped & twist_with_cov_raw)
{
  ...
  // std::cout << "publishing twist with cov msg." << std::endl;
